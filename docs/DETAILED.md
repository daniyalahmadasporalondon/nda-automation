...
**Detection floor.** Inbound NDA detection always uses a hardcoded **keyword floor**; `NDA_GMAIL_CUSTOM_TERMS_ENABLED` (default off) is a hard safety gate. When ON, the admin-edited "Signal Terms" (Gmail `inbound_search_terms`) are ADDED on top of the floor as additive, literal case-insensitive substrings for the deterministic **content** scorer only — never added to the Gmail fetch query (broadening the fetch is the inbox-storm vector) and never triggering synchronous review. Terms are validated + capped (≥3 chars, generic-word denylist, regex-neutralized, ≤25 terms) at save and read.
...
| `NDA_GMAIL_CUSTOM_TERMS_ENABLED` | admin Signal Terms added to the content scorer only (never the fetch query) | **off** | no |
...
- **Detection floor is always on** (hardcoded keywords). To add signals, enable `NDA_GMAIL_CUSTOM_TERMS_ENABLED` (default off) and edit "Signal Terms" (`inbound_search_terms`) in Gmail settings — they feed the deterministic **content scorer only**, never the fetch query, and are validated/capped (≥3 chars, denylist, ≤25). This is the safe knob; broadening the fetch query is the inbox-storm vector and is deliberately not exposed.
...
- **Live vs inert today:** the intake classifier (Flash) and attachment selector (Pro) are live; the custom-terms flag is **inert by default** (off) so behaviour is byte-for-byte the hardcoded floor until an admin turns it on. Inbound auto-review does not exist — review is on-demand only.
