...
def handle_generate_nda(handler) -> None:
    ...
    try:
        entity_id, intake = _intake_from_payload(payload)
    ...
    try:
        result, matter, artifact = _generate(entity_id, intake, owner_user_id=owner_user_id)
...
def _generate(entity_id: str, intake: CounterpartyIntake, *, owner_user_id: str):
    ...
    result = nda_generation.generate_nda_for_entity(entity_id, intake)
...
def _intake_from_payload(payload: dict[str, Any]) -> tuple[str, CounterpartyIntake]:
    """Map the draft-ui Generate payload to (entity_id, CounterpartyIntake).
    ...
    ``governing_law_override`` is accepted but rejected when non-empty: in v1 the
    governing law is fixed by the chosen signing entity (the registry's approved
    Playbook option), so an override cannot be honoured without putting the draft
    off its entity's registered position.
    """

    if not isinstance(payload, dict):
        raise _PayloadError("Request body must be a JSON object.")

    _reject_governing_law_override(payload)
...
def _reject_governing_law_override(payload: dict[str, Any]) -> None:
    """v1 fixes governing law per signing entity, so a non-empty override is a
    client error (silently ignoring it would mislead the caller into thinking the
    override took effect)."""
    override = payload.get("governing_law_override")
    if isinstance(override, str) and override.strip():
        raise _PayloadError(
            "governing_law_override is not supported: the governing law is fixed by the "
            "chosen signing entity."
        )
