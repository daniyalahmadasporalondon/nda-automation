...
def _check_signatures(
    ...
    partial_matches = merge_paragraphs(
        _signature_evidence_paragraphs(paragraphs, signature_patterns),
    ...
    if party_markers >= 2 and title_markers >= 2 and date_markers >= 1:
        return attach_structure_context(
            _match(clause, "Execution block appears to include both parties, titles, and a date.", partial_matches),
            review_context,
            context_concepts,
        )
    if partial_matches:
